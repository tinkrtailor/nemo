...
- FR-22: The auth-sidecar runs as a **K8s native sidecar** — i.e. an `initContainer` with `restartPolicy: Always` (k8s 1.29+ GA, supported on the k3s versions nautiloop pins). This gives us:
  1. **Ordering for free:** the sidecar's `startupProbe` on `:9093/healthz` gates the agent container start. The `/healthz` handler returns `503 starting` until ALL four proxy ports (`:9090` model, `:9091` git SSH, `:9092` egress, `:9093` health) are bound and listening, then flips to `200 ok`. So the startupProbe is gating on **full** sidecar readiness, not just on the health server having bound its own port.
...
- FR-24: Each agent job shall be a K8s Job with `restartPolicy: Never`, **one regular container** (`agent`), and **two init containers**: `init-iptables` (egress enforcement, runs and exits) and `auth-sidecar` (the K8s native sidecar — `restartPolicy: Always` initContainer per FR-22, runs for the agent's lifetime and is auto-terminated when the agent exits). If the `nemo-registry-creds` Secret exists (created by Terraform when `image_pull_secret_dockerconfigjson` is provided, see FR-52), the Job template shall include `imagePullSecrets: [{ name: nemo-registry-creds }]`. Otherwise, no `imagePullSecrets` (public images or pre-pulled).
