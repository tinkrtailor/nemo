...
fn engineer_color(name: &str) -> &'static str {
    ...
    let colors = [
        "#1B6B5A", "#3B7BC0", "#C4841D", "#8B5CF6", "#C4392D", "#2D7A4F", "#E8A838", "#6366F1",
        "#0EA5E9", "#D946EF",
    ];
...
pub fn render_detail(data: &DetailData, csrf_token: &str) -> Markup {
    ...
    layout(
        &format!("{} — nautiloop", spec_filename(&r.spec_path)),
        "grid",
        false,
        csrf_token,
        html! {
...
pub fn render_spec_history(
    ...
    layout(
        &format!("{} — nautiloop", spec_filename(spec_path)),
        "grid",
        false,
        csrf_token,
        html! {
...
pub fn render_stats(data: &StatsData, csrf_token: &str) -> Markup {
    let max_daily = data
        .daily_series
        .iter()
        .map(|d| d.started.max(d.converged).max(d.failed))
        .max()
        .unwrap_or(1)
        .max(1);
